...
import { ElkonThemes } from './colors'

export default function getTheme({ style, name, soft = false }) {
  // Usage: `pick({ light: "lightblue", dark: "darkblue" })`
  const pick = options => options[style]

  const vitesse = (key: keyof typeof ElkonThemes, op = '') => pick({ light: ElkonThemes[key][1] + op, dark: ElkonThemes[key][0] + op })
  ...
  const theme = {
    ...
    colors: {
      ...
      'terminal.ansiBlack': pick({ light: ElkonThemes.background[0], dark: ElkonThemes.foreground[1] }),
      ...
      'terminal.ansiWhite': pick({ light: ElkonThemes.foreground[0], dark: ElkonThemes.foreground[0] }),
